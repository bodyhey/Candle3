...
declare var vertexShaderSource1: string;
declare var fragmentShaderSource1: string;
declare var vertexShaderSource2: string;
declare var fragmentShaderSource2: string;
declare var vertexShaderSource2d: string;
declare var fragmentShaderSource2d: string;
...
class GLWidget {
    ...
    m_shaderProgram2d = new ShaderProgram2d(vertexShaderSource2d, fragmentShaderSource2d);
    m_shaderDrawables = [
    ...
    m_zoomDistance;
    m_lastPos = {x: 0, y: 0};
    m_lookAt = glMatrix.vec3.fromValues(0, 0, 0);
...
//        glMatrix.mat4.invert(m, this.m_projectionMatrix);
    ...
    updateView()
    {
        ...
        const eye = glMatrix.vec3.fromValues(
            Math.cos(angX) * Math.sin(angY),
            Math.sin(angX),
            Math.cos(angX) * Math.cos(angY)
        );
        ...
        this.cube.updateRotation(
            eye[0], eye[1], eye[2],

            // Math.cos(angX) * Math.sin(angY) * Math.PI,
            // Math.sin(angX)* Math.PI,
            // Math.cos(angX) * Math.cos(angY)* Math.PI
        );
        gl.viewport(0, 0, this.width(), this.height());
    }

    fitDrawable(drawable: any)
    {
        // stopViewAnimation();
    ...
    paintEvent() {
        ...
        //        if (this.m_zBuffer)
        const bindEverything = (program: ShaderProgram) => {
            program.bind();
...
//        gl.disable(gl.BLEND);
    ...
    private generatePaletteTexture() {
        var colorPalette = [
            1.0, 0.0, 0.0, 1.0, // Czerwony
            1.0 / 57, 1.0 / 255, 0, 1.0, // HEX 39FF14
            1.0, 1.0, 0.0, 1.0, // Żółty
            1.0, 0.0, 1.0, 1.0, // Fioletowy
            0.0, 1.0, 1.0, 1.0, // Turkusowy
            1.0, 1.0, 1.0, 1.0, // Biały
            0.6, 1.0, 0.6, 1.0, // Jasnozielony
            0.6, 0.6, 1.0, 1.0, // Jasnoniebieski
